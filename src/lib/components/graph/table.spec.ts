...
describe('tableColumns', () => {
	...
	it('keeps a named set out of an unnamed column above 16 symbols too', () => {
		const digit = { name: 'digit', set: CharSet.range('0', '9') };
		const letter = { name: 'letter', set: CharSet.range('A', 'Z').union(CharSet.range('a', 'z')) };
		// 63 symbols: 0 moves to 1 on digit and on _, which merge unless digit keeps its name.
		const a: Automaton = {
			states: states(2),
			transitions: [
				{ id: 0, from: 0, to: 1, label: digit.set },
				{ id: 1, from: 0, to: 1, label: CharSet.of('_') },
				{ id: 2, from: 1, to: 1, label: letter.set }
			],
			start: 0
		};
		expect(headers(a)).toEqual(['0–9,_', 'A–Z,a–z']);
		expect(headers(a, { names: [digit, letter] })).toEqual(['digit', 'letter', '_']);
		expect(headers(a, { names: [letter] })).toEqual(['0–9,_', 'letter']);
		// Named sets made of several merged classes keep their names as well.
		const b: Automaton = {
			states: states(2),
			transitions: [
				{ id: 0, from: 0, to: 1, label: CharSet.range('A', 'Z') },
				{ id: 1, from: 0, to: 1, label: CharSet.range('a', 'z') },
				{ id: 2, from: 0, to: 1, label: CharSet.of('_') },
				{ id: 3, from: 1, to: 1, label: digit.set }
			],
			start: 0
		};
		expect(headers(b)).toEqual(['0–9', 'A–Z,_,a–z']);
		expect(headers(b, { names: [digit, letter] })).toEqual(['digit', 'letter', '_']);
		const cols = tableColumns(b, { names: [digit, letter] });
		expect(cols[1].set.equals(letter.set)).toBe(true);
	});

	it('keeps a named set made of several merged symbols whole at 16 symbols or fewer', () => {
		const bit = { name: 'bit', set: CharSet.of('01') };
		const a: Automaton = {
			states: states(2),
			transitions: [
				{ id: 0, from: 0, to: 1, label: CharSet.of('0') },
				{ id: 1, from: 0, to: 1, label: CharSet.of('1') },
				{ id: 2, from: 0, to: 1, label: CharSet.of('2') }
			],
			start: 0
		};
		expect(headers(a)).toEqual(['0', '1', '2']);
		expect(headers(a, { names: [bit] })).toEqual(['bit', '2']);
	});
