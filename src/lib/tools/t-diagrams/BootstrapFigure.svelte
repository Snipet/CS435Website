<!--
	...
	Both arrangements are rendered and a container query shows one: the slide's,
	or, in a box narrower than FIGURE_NARROW_BELOW px, the stacked one, so the
	labels stay at MIN_LABEL_PX or more on a phone.
-->
<script lang="ts">
	import {
		bootstrapFigure,
		...
		WANT,
		WANT_LABEL_SIZE,
		type BootstrapFigure as Figure,
		type WalkStep
	} from './walkthrough';
	...
	const uid = $props.id();
	const figures = [bootstrapFigure(undefined, 'slide'), bootstrapFigure(undefined, 'narrow')];
	const PAD = FIGURE_PAD;

	const label = $derived.by(() => {
		const parts = [`Given machine M and language L, want ${formatT(WANT)}. But tough directly!`];
		figures[0].rows.forEach((r, i) => {
...
{#snippet drawing(fig: Figure)}
	{@const vw = figureViewWidth(fig)}
	{@const vh = fig.height + PAD.top + PAD.bottom}
	{@const tip = `${uid}-${fig.layout}-tip`}
	<svg
		class={['figure', fig.layout]}
	...
	>
		...
		<text
			...
			x={fig.arrow.label.x}
			y={fig.arrow.label.y}
			font-size={WANT_LABEL_SIZE}
			text-anchor={fig.arrow.label.anchor}>Want this</text
		>
		<TShape
			t={WANT}
			geom={fig.goal.geom}
...
<div class="bootstrap-figure">
	{#each figures as fig (fig.layout)}
		{@render drawing(fig)}
	{/each}
</div>
...
<style>
	...
	/* FIGURE_NARROW_BELOW (walkthrough.ts): below it the slide's labels would drop under 15 px. */
	@container (width < 457px) {
		.figure.slide {
			display: none;
		}
		.figure.narrow {
